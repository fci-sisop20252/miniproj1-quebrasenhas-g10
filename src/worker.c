...
int increment_password(char *password, const char *charset, int charset_len, int password_len) {
    ...
    for (int i = password_len - 1; i >= 0; i--) {
        ...
        while (current_char_index<charset_len &&charset[current_char_index]!=password[i])
        {
            current_char_index++;
        }

        if (current_char_index >= charset_len) {
            return 0; 
        }

        if (current_char_index < charset_len - 1) {
            password[i] = charset[current_char_index + 1];
            return 1;
        } else {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (passwords_checked % PROGRESS_INTERVAL == 0 && passwords_checked > 0) {
            if (check_result_exists()) {
                printf("[Worker %d] Outro worker encontrou a senha. Parando...\n", worker_id);
                break;
            }
            printf("[Worker %d] Progresso: %lld senhas verificadas\n", worker_id, passwords_checked);
        }
